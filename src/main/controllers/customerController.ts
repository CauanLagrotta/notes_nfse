...
export async function getCustomers(page: number = 1, limit: number = 10) {
  try {
    // calculo do numero de registros
    const skip = (page - 1) * limit;

    // consulta os clientes paginados
    const customers = await prisma.customer.findMany({
      skip,
      take: limit,
      orderBy: {
        customer_name: "asc", // ordenar os clientes pelo nome em ordem alfabetica crescente
      },
    });

    // consulta o total de clientes
    const totalCustomers = await prisma.customer.count(); // total de clientes
