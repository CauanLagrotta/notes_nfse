...
export function TaxInvoicesDone() {
    ...
    try {
      ...
      const combinedData: CustomerInvoiceProps[] = result.map(
        (item: CustomerInvoiceProps) => ({
          ...
          tax_status: item.tax_status && "Concluído",
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const filteredResult = result.filter(
        (item: CustomerInvoiceProps) => item.tax_status === "Concluído",
      );
  ...
  const regress_status = async (id: number, tax_status: string) => {
    const previous_status =
      tax_status === "Concluído" ? "Pendente" : "Pendente";

    try {
      await window.api.updateStatus(id, previous_status);
      getAllTaxes();
    } catch (error) {
